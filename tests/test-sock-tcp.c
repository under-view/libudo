...
{
	struct udo_sock_tcp *server = NULL;

	struct udo_sock_tcp_server_create_info server_info;

	server_info.ipv6 = 0;
	server_info.port = 7777;
	server_info.ip_addr = "127.0.0.1";
	server_info.connections = 1;
	server = udo_sock_tcp_server_create(NULL, &server_info);
	assert_non_null(server);

	udo_sock_tcp_destroy(server);
}
...
{
	struct udo_sock_tcp *client = NULL;

	struct udo_sock_tcp_client_create_info client_info;

	client_info.ipv6 = 0;
	client_info.port = 7777;
	client_info.ip_addr = "127.0.0.1";
	client = udo_sock_tcp_client_create(NULL, &client_info);
	assert_non_null(client);

	udo_sock_tcp_destroy(client);
}
...
p_test_sock_tcp_accept_connect_client (void)
{
	...
	err = udo_sock_tcp_client_connect(client);
...
test_sock_tcp_accept_connect (void UDO_UNUSED **state)
{
	...
	client_sock = udo_sock_tcp_server_accept(server, NULL);
...
p_test_sock_tcp_send_recv_client (void)
{
	...
	size = udo_sock_tcp_client_send_data(client, buffer, sizeof(buffer), 0);
...
test_sock_tcp_send_recv (void UDO_UNUSED **state)
{
	...
	memset(buffer, 'T', sizeof(buffer));
	udo_sock_tcp_recv_data(client_sock, buffer_two, sizeof(buffer_two), 0);
	assert_memory_equal(buffer, buffer_two, sizeof(buffer));

	/* Test client disconnects */
	memset(buffer_two, 0, sizeof(buffer_two));
	recv = udo_sock_tcp_recv_data(client_sock, buffer_two, sizeof(buffer_two), 0);
...
test_sock_tcp_get_fd (void UDO_UNUSED **state)
{
	...
	sock_fd = udo_sock_tcp_get_fd(NULL);
	assert_int_equal(sock_fd, -1);

	sock_fd = udo_sock_tcp_get_fd(client);
...
test_sock_tcp_get_ip_addr (void UDO_UNUSED **state)
{
	...
	ip_addr = udo_sock_tcp_get_ip_addr(NULL);
	assert_null(ip_addr);

	ip_addr = udo_sock_tcp_get_ip_addr(server);
...
test_sock_tcp_get_port (void UDO_UNUSED **state)
{
	...
	port = udo_sock_tcp_get_port(NULL);
	assert_int_equal(port, -1);

	port = udo_sock_tcp_get_port(client);
